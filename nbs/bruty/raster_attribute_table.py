...
If this logic does not agree with what we discussed please let me know.
    ...
    """
    ds = gdal.Open(tile_filename)
    band = ds.GetRasterBand(3)  # FIXME - find the contributor band
    ...
    print("Faking the sensitive flag")
    # FIXME - using sensitive = false for the moment
    for n, nbs_id in enumerate(used_contributors):
        if nbs_id > 0:
            rec = meta_records[nbs_id]
            contributor_metadata[rec['from_filename']] = {'record': rec, 'sensitive': False, 'index': int(nbs_id)}  # ugh, int(nbs_id) to cast out of numpy.int32 for use later
        else:
