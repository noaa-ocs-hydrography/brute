...
def retrieve_meta_for_Ehydro_notable(highresfolder, ehydrofolder, district, metafile, df_export_to_csv):
    ...
    for basename in g1:
        ...
        if os.path.isfile(xmlfilename):        
            ...
            meta_xml = {}
        meta = e_t.parse_ehydro_xyz(f, meta_source = 'xyz', version= district, default_meta = '')#parse_ehydro_xyz(infilename, meta_source = 'xyz', version='CEMVN', default_meta = '')#eH_p.parse_ehydro_xyz(f)
        ...
        m2c.write_meta2csv([merge2],metafile)#could also try merged_meta, but merge2 outputs a list within a cell if inputs from text and xml for the same variable are different
...
def test_cesam_ccom_defaultnames():    
    district = 'CESAM'
    highresfolder = r"N:\New_Directory_1\GulfCoast\USACE\ehydro\EasternGulf\downloads\CESAM"
    ehydrofolder = r"N:\New_Directory_1\GulfCoast\USACE\ehydro\EasternGulf\downloads\CESAM"#r'N:\New_Directory_1\GulfCoast\Mississippi\USACE\ehydro\tst'
    mv_to_dir = r"N:\New_Directory_1\GulfCoast\USACE\ehydro\EasternGulf\CESAM"#D:\NBS_Data\PBG_GulfCoast_PR_VI\Mississippi\USACE\E-Hydro\download\files\downloads\CEMVN"
    ...
    merged_meta1, merged_dataframe = retrieve_meta_for_Ehydro_notable(highresfolder, ehydrofolder, district, metafile, df_export_to_csv)
    ...
    for filenamepath in Ax_row_index:
        ...
        if merged_dataframe.at[filenamepath,'script: from_vert_key']== 'MLLW':
            ...
            #Vertical datum unknown
                S_f_d.look_for_Ehydro_datum_folders(filenamepath, merged_dataframe.at[filenamepath, 'script: from_vert_key'], mv_to_dir )
            else:
