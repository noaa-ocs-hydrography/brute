...
def regionPath(root: str, folder: str) -> dict:
    ...
    for row in temp[1:]:
        ...
        bounds = row[-1]
        #ignoring the autoconstruct path to folder and just using the folder input from config
        regions.append((regionName, [downloads, bounds]))#regions.append((regionName, [dwnlds, bounds]))
