...
def getShpRast(file: str, y, pixel_size=1, nodata=255):
    """
    Import shapefile
...
def write_raster(raster_array, gt, data_obj, outputpath, dtype=_gdal.GDT_UInt32, options=0, color_table=0, nbands=1,
                 nodata=False):
    """
    Directly From:
    "What is the simplest way..." on GIS Stack Exchange [Answer by 'Jon'
...
def alignGrids(bag: list, tif: list, maxVal: int, targs: list):
    """
...
def rePrint(grids: list, ugrids: list, maxVal, ioVal: Union[int, bool], debug: Union[int, bool] = 0):
    """
    ...
    if z > 1:
        ...
        for ySlice in range(chunkGrid.shape[0]):
            for xSlice in range(chunkGrid.shape[1]):
                ...
                unitedBag[tile.yBMin:tile.yBMax, tile.xBMin:tile.xBMax] = newBag[tile.yIMin:tile.yIMax,
                                                                          tile.xIMin:tile.xIMax]
                unitedUncr[tile.yBMin:tile.yBMax, tile.xBMin:tile.xBMax] = newUncr[tile.yIMin:tile.yIMax,
                                                                           tile.xIMin:tile.xIMax]
                unitedPre[tile.yBMin:tile.yBMax, tile.xBMin:tile.xBMax] = preBag[tile.yIMin:tile.yIMax,
                                                                          tile.xIMin:tile.xIMax]
