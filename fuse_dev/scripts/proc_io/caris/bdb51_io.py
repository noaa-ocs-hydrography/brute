...
class bdb51:
    ...
    def _start_env(self, port):
        ...
        try:
            db = subprocess.Popen(args, creationflags=subprocess.CREATE_NEW_CONSOLE, close_fds=True)
        except:
    ...
    def form_connection(self, host='localhost', port=0):
        ...
        for response in socket.getaddrinfo(host, self.port, socket.AF_UNSPEC, socket.SOCK_STREAM, 0, socket.AI_PASSIVE):
            af, socktype, proto, canonname, sa = response
    ...
    def domath(self):
        ...
        self.data = self.conn.recv(1024)
        print('Received {}'.format(repr(self.data)))
