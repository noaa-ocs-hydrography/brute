...
if __name__ == '__main__':
    ...
    logger.info(f'starting BPS processing at {start_time}')
    ...
    for config_index, config_filename in enumerate(config_filenames):
        ...
        for survey_index, survey_directory in enumerate(survey_directories):
            logger.info(f'config {config_index + 1} of {len(config_filenames)}, ' +
            ...
            if xyz_filenames is None or len(xyz_filenames) == 0:
                ...
                for xyz_filename in xyz_filenames:
                    try:
                        logger.info(f'{datetime.now()}: processing {xyz_filename}', end=', ')
                        bps_processor.process(xyz_filename)
                    except Exception as error:
                        _, _, error_traceback = sys.exc_info()
                        logger.error(f'processing error: {error.__class__.__name__} {error} ' +
                                     f'({os.path.split(error_traceback.tb_frame.f_code.co_filename)[1]}:{error_traceback.tb_lineno})')
                logger.info(f'completed survey {survey_index + 1} of {len(survey_directories)}; ' +
    ...
    logger.info(f'completed BPS processing at {end_time} (took {end_time - start_time})')
    # wx_frame.close()
