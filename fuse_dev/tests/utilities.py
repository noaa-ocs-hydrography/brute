...
NOAA_BAG_INPUT_ROOT = os.path.join(INPUT_ROOT, 'NOAA_bag')
NOAA_BPS_INPUT_ROOT = os.path.join(INPUT_ROOT, 'NOAA_bps')
OUTPUT_ROOT = os.path.join(TESTING_DIRECTORY, 'output')

USACE_CONFIG_ROOT = os.path.join('data', 'USACE')
NOAA_BAG_CONFIG_ROOT = os.path.join('data', 'NOAA_bag')
NOAA_BPS_CONFIG_ROOT = os.path.join('data', 'NOAA_bps')
...
def process_NOAA_raster(survey_name: str, interpolation_method: str, output_type: str) -> [str]:
    ...
    config_path = os.path.join(NOAA_BAG_CONFIG_ROOT, f'{survey_name}_{interpolation_method}_{output_type}.config')
...
def process_NOAA_points(survey_name: str, interpolation_method: str, output_type: str) -> [str]:
    for filename in glob(os.path.join(OUTPUT_ROOT, f'{survey_name}.*')):
        os.remove(filename)

    config_path = os.path.join(NOAA_BPS_CONFIG_ROOT, f'{survey_name}_{interpolation_method}_{output_type}.config')
