...
def upload_to_azure(local_path: str, remote_path: str, credentials: str, overwrite: bool = False):
    ...
    os.chdir(azcopy_dir)
    os.system(f'{azcopy_filename} copy "{local_path}" "{remote_path}?{credentials}" --overwrite={str(overwrite).lower()}' +
              ' --follow-symlinks --recursive --from-to=LocalBlob --blob-type=BlockBlob --put-md5')
...
if __name__ == '__main__':
    ...
    upload_to_azure(os.path.join(local_data_path, 'reference'), remote_data_path, credentials, overwrite=True)
