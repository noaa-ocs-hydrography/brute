...
extract_ehydro_meta_class_CESAM.py
...
class XYZMetaReader(object):
    ...
    def parse_ehydro_xyz(self, infilename: str, meta_source: str = 'xyz', version: str = 'CESAM',
    ...
    for aline in lines:
        if aline != '':
            ...
            if 'horiz_sys' in metadata and 'COORDINATE SYSTEM' in metadata:
                horiz_datum = metadata['horiz_sys'].split(' STATE PLANE')[0] + \
                              metadata['COORDINATE SYSTEM'].split('83 DATUM')[-1].strip('.')
                ...
                for key in p_usace_xml.SOURCEPROJECTION_dict:
                    if key.upper() in metadata['from_horiz_datum']:
                        metadata['from_fips'] = p_usace_xml.convert_tofips(p_usace_xml.SOURCEPROJECTION_dict,
                                                                           " ".join(key.split()))
    return metadata
...
def use_best_spcs(merged_meta:dict) ->dict:
    """
    use_best_spcs()
    take merged_meta dictionary and use 'ehydro_from_fips'
    for the from_fips value.
    ...
    *A conflict in the SPCS codes within the xml alone will not make
    ...
    -------   
    merged_meta: dict
    #if ehydro_from_fips was calculated by the pickle SPCS checker then
    then from_fips is overwritten as it otherwise there may be two values
    in the output, it is the winner if it was calculated.
    
    """
    #meta_from_ehydro: dict, 
    #meta_from_ehydro['ehdyro_from_fips'] = meta_from_ehydro['from_fips']
    #if 'from_fips' in meta_from_ehydro:
